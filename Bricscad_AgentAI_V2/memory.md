...
- v2.36.1 GOLD [FOREACH UI THREAD SAFETY] - `ForeachTool` rozpoznaje narzedzia wymagajace glownego watku UI (`ImportLayoutTemplateTool`, `ExportLayoutTemplateTool`, `PlotLayoutTool`, `PublishToPdfTool`) i dispatchuje je przez `AgentControl.Invoke`, aby unikac crashy Teigha/BricsCAD przy pracy z layoutami z watku worker. Dodano `StopOnError=true` jako domyslne przerwanie petli po pierwszym bledzie oraz stub `LightChatControlWpf.g.cs` dla bezposredniej kompilacji bez pelnego WPF MSBuild pipeline.
...
- Filar 4 (v2.34.6): CircuitBreakerState per-profil z ConcurrentDictionary, prog domyslny 3. DelegateTaskTool: blokada na poczatku try, RecordFailure w 4 miejscach, Reset po Accept. ToolOrchestrator: heurystyczny rollback detection (BŁĄD/Abort/przerwan).
...
- Dodano klas� DTO BielikViewDef oraz obs�ug� NOD (NodManager).
...
- Stworzono integracj� z Agentem AI V2: ReadViewDefinitionsTool (#layout), pobieraj�cy s�ownik BIELIK_DRUK_WIDOKI.
- Zaktualizowano prompt systemowy (system_prompt_layout.txt).
### [STAN_SYSTEMU]
- Kompilacja obu projekt�w przebieg�a bez b��d�w. Narz�dzia AI potrafi� czyta� NOD przez nowo dodane narz�dzie.
### [BLOKADY / PROBLEMY]
- Brak.
### [KOLEJNY_KROK]
