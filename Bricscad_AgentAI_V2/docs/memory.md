...
## [v2.28.2] 2026-06-07T11:40:00+02:00 - Poprawa interfejsu QA i dostępu Agenta do kodu źródłowego
### [ZREALIZOWANO]
- Naprawiono krytyczny błąd kasowania się historii konwersacji w oknie QA poprzez wywołanie `QASessionManager.SaveSession()` natychmiast po udanej odpowiedzi z modelu, a przed asynchronicznym odświeżeniem GUI (`LoadSessions()` i `LbSessions_SelectedIndexChanged`). Zapobiega to nadpisywaniu nowej sesji pustym stanem z dysku (Race Condition).
...
- Zaktualizowano generatory `system_prompt_auditor.txt` i plik awaryjny w `QASessionManager.cs` o wyraźne instrukcje dające QA pełny dostęp odczytu do warstwy C# w BricsCAD_AgentAI_V2.
### [STAN_SYSTEMU]
- UI QA stabilne, a Agent w pełni świadomy swojej mocy analitycznej całego kodu źródłowego.
### [BLOKADY / PROBLEMY]
- Brak.
### [KOLEJNY_KROK]
...
### [ZREALIZOWANO]
- Wdro�ono narz�dzie RunToolTestTool.cs pozwalaj�ce Agentowi QA (M�zg) na niezale�ne i bezpieczne uruchamianie statycznych test�w narz�dzi CAD poprzez wstrzykiwanie kontrolowanego JSON i �apanie wyj�tk�w.
- Zmodyfikowano ToolConfigManager.cs, rozszerzaj�c pul� dozwolonych narz�dzi AuditorProfile o RunToolTest, WriteQAReport oraz DelegateTaskToAntigravity.
- Zaktualizowano systemowy prompt dla Supervisora i Auditora: Supervisor jest teraz w pe�ni �wiadomy mocy diagnostycznej Agenta QA i potrafi zapyta� u�ytkownika o zgod� na przekazanie mu problem�w za pomoc� DelegateTaskTool.
- Wprowadzono architektur� delegowania zada� do zewn�trznego agenta kodowania (Antigravity) poprzez zapisywanie zlece� w folderze Autotesty/TasksForAntigravity/.
### [STAN_SYSTEMU]
- System potrafi samodzielnie testowa� sw�j kod, wychwytywa� b��dy C# bez crashowania �rodowiska CAD, generowa� profesjonalne raporty QA i wystawia� zadania dla zewn�trznych agent�w deweloperskich.
### [BLOKADY / PROBLEMY]
- Wymagana kompilacja projektu poprzez natywne narz�dzia MSBuild / Visual Studio z powodu ogranicze� �rodowiska CLI wzgl�dem biblioteki Newtonsoft.Json.
### [KOLEJNY_KROK]
- Testowanie w �rodowisku produkcyjnym i ewentualne tworzenie zautomatyzowanych ci�g�w test�w dla M�zgu.
...
## [v2.28.5] 2026-06-08T00:30:00+02:00 - Rozbudowa interfejsu LISP oraz poprawa wywo�ywania skrypt�w LISP
### [ZREALIZOWANO]
- Wdro�ono autouzupe�nianie UI (klasa AgentControl.cs) obs�uguj�ce skrypty LISP po wpisaniu znaku '%' poprzez bezpo�rednie wstrzykiwanie LispManager.LoadAllLisps() do opcji podpowiedzi.
- Zmodyfikowano metod� ExecuteLispFromTemp w AgentControl.cs w celu pe�nego wykonania zdefiniowanego skryptu AutoLISP. Od teraz aplikacja najpierw wczytuje LISP (doc.SendStringToExecute("(load ...) ")), a nast�pnie automatycznie uruchamia sam� komend� (doc.SendStringToExecute("lispId ")).
- Zaktualizowano dokumentacj� USER_GUIDE.md dodaj�c informacje o nowym podpowiadaniu skrypt�w w interfejsie.
### [STAN_SYSTEMU]
- Autouzupe�nianie LISP dzia�a poprawnie, a polecenia wywo�ywane przez Agenta lub UI poprzez '%skrypt' odpalaj� w�a�ciwe akcje bezpo�rednio w rysunku.
### [BLOKADY / PROBLEMY]
